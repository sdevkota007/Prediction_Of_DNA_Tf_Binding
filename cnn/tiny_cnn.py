...
def load_ENCODE_k562_dataset(dataset_num):
    ...
    for get_test in range(0, 2):
        ...
        for line in f:
            ...
            if match:
                ...
                dna_string = match.group(4)  # the sequence of DNA base pairs in range
                bound = int(match.group(
                    5))  # whether or not the transcription factor did bind to this DNA sequence (1 if bound, 0 if not bound)
                if len(dna_string) == 101:
                    if num_bases < 1:
    ...
    validation_size = int(train_features.shape[
...
def main(_):
    ...
    for dataset_num in range(0, len(_datasets)):
        ...
        cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=y_conv,
                                                                               labels=y_))  # tf.nn.softmax_cross_entropy_with_logits(logits = yPredbyNN, labels=Y)
        ...
        while stop_condition is None:
            if i % 1000 == 0:
                ...
                while _validation_epochs_completed - prev_validation_epochs_completed == 0:  # do in mini batches because single GTX970 has insufficient memory to test all at once
                    ...
                    if pred_validation_labels is None:
                        pred_validation_labels = y_conv_softmax.eval(
                            feed_dict={x: validation_batch[0], y_: validation_batch[1]})
                        true_validation_labels = validation_batch[1]
                    else:
                        pred_validation_labels = numpy.vstack([pred_validation_labels, y_conv_softmax.eval(
                            feed_dict={x: validation_batch[0], y_: validation_batch[1]})])
                ...
                print(
                    ...
                    _datasets[dataset_num], dataset_num, _train_epochs_completed, i, time.time() - t0, roc_auc,
                    perc_chg_auc))
                t0 = time.time()
